...
do
    ...
    # ==================== GPU 2 ====================
    # 修改：已替换为 卡1 的参数 (Qwen2.5, pairwise=0.01)
    if is_enabled 2; then
        echo "[Round $ROUND] 正在启动 GPU 2 (使用卡1配置)..."
    ...
    # ==================== GPU 5 ====================
    # 修改：已确认使用 卡6 的参数 (Qwen2.5, pairwise=0.1)
    if is_enabled 5; then
        echo "[Round $ROUND] 正在启动 GPU 5 (使用卡6配置)..."
