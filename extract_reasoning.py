import re
import os
import argparse

def extract_final_answer(log_file_path):
    """
    从日志文件中提取final_answer标记下一行的内容作为最终答案
    
    参数:
        log_file_path: 日志文件路径
        
    返回:
        str: 最终答案内容，如果未找到则返回None
    """
    try:
        with open(log_file_path, 'r', encoding='utf-8') as file:
            content = file.read()
            
            # 使用正则表达式查找final_answer标记及其后的内容
            # 匹配格式为：------------------------------final_answer------------------------------\n[答案内容]\n
            pattern = r'-+--final_answer---+\n(.*?)\n'
            matches = re.search(pattern, content)
            
            if matches:
                final_answer = matches.group(1).strip()
                return final_answer
            else:
                print(f"警告: 在{log_file_path}中未找到final_answer标记")
                return None
    except Exception as e:
        print(f"处理{log_file_path}时出错: {str(e)}")
        return None

def extract_boxed_content(text):
    """整提取\boxed{}内容，支持嵌套大括号和特殊命令
    优先返回最后一个匹配的boxed内容
    """
    pattern = r'\\boxed\{((?:[^{}]|\{[^{}]*\})*)\}'
    matches = re.findall(pattern, text)
    return matches[-1].strip() if matches else None

def extract_reasoning_and_answer(log_file_path):
    """
    从日志文件中提取推理路径和最终答案
    
    参数:
        log_file_path: 日志文件路径
        
    返回:
        tuple: (推理路径, 最终答案)
    """
    try:
        with open(log_file_path, 'r', encoding='utf-8') as file:
            content = file.read()
            
            # 查找文件末尾的推理路径部分，从"Reasoning Path:"到文件结尾
            reasoning_match = re.search(r'Reasoning Path:(.*)', content, re.DOTALL)
            
            if not reasoning_match:
                print(f"警告: 在{log_file_path}中未找到推理路径")
                return None, None
                
            reasoning_path = reasoning_match.group(1).strip()
            
            # 使用新的boxed提取函数提取答案（优先最后一个boxed）
            final_answer = extract_boxed_content(reasoning_path)
            
            # 如果未找到有效的boxed答案，使用最后一行作为答案
            if final_answer is None:
                lines = reasoning_path.strip().split('\n')
                if lines:
                    final_answer = lines[-1].strip()
                    print(f"在{log_file_path}中未找到有效的\\boxed{{}}格式答案，使用最后一行作为答案")
                else:
                    print(f"警告: 在{log_file_path}中未找到最终答案")
                    final_answer = None
                
            return reasoning_path, final_answer
    except Exception as e:
        print(f"处理{log_file_path}时出错: {str(e)}")
        return None, None

def extract_case_info(filename, dataset_name="amc23"):
    """
    从日志文件名中提取数据集名称和案例ID
    
    参数:
        filename: 格式为case_{case_id}.txt的日志文件名
        dataset_name: 数据集名称，默认为amc23
    
    返回:
        tuple: (数据集名称, 案例ID)
    """
    # 匹配 case_0001.txt 格式的文件名
    match = re.search(r'case_(\d+)\.txt', filename)
    if match:
        case_id = match.group(1)
        return dataset_name, case_id
    else:
        print(f"警告: 无法解析文件名 {filename}，使用默认值")
        return dataset_name, "unknown"

def process_log_files(input_path, reasoning_output_path, answer_output_path, dataset_name="amc23"):
    """
    处理日志文件并提取推理路径和最终答案
    
    参数:
        input_path: 日志文件或包含日志文件的目录路径
        reasoning_output_path: 保存推理路径的目录路径
        answer_output_path: 保存最终答案的目录路径
        dataset_name: 数据集名称
    """
    # 如果输出目录不存在则创建
    os.makedirs(reasoning_output_path, exist_ok=True)
    os.makedirs(answer_output_path, exist_ok=True)
    
    if os.path.isdir(input_path):
        # 处理目录中的所有日志文件
        for root, _, files in os.walk(input_path):
            for file in files:
                if file.endswith('.txt') and file.startswith('case_'):
                    file_path = os.path.join(root, file)
                    process_single_file(file_path, reasoning_output_path, answer_output_path, dataset_name)
    else:
        # 处理单个日志文件
        process_single_file(input_path, reasoning_output_path, answer_output_path, dataset_name)

def process_single_file(file_path, reasoning_output_path, answer_output_path, dataset_name="amc23"):
    """
    处理单个日志文件并将结果写入单独的输出文件
    
    参数:
        file_path: 日志文件路径
        reasoning_output_path: 保存推理路径的目录
        answer_output_path: 保存最终答案的目录
        dataset_name: 数据集名称
    """
    # print(f"正在处理文件: {file_path}")
    
    reasoning_path, path_answer = extract_reasoning_and_answer(file_path)
    final_answer = extract_final_answer(file_path)
    
    filename = os.path.basename(file_path)
    extracted_dataset, case_id = extract_case_info(filename, dataset_name)
    
    # print(f"提取的信息 - 数据集: {extracted_dataset}, 案例ID: {case_id}")
    
    # 创建输出文件名
    reasoning_filename = f"{extracted_dataset}_case_{case_id}_reasoning.txt"
    answer_filename = f"{extracted_dataset}_case_{case_id}_answer.txt"
    
    # print(f"推理输出文件: {reasoning_filename}")
    # print(f"答案输出文件: {answer_filename}")
    
    # 将推理路径保存到单独的文件
    if reasoning_path:
        reasoning_file_path = os.path.join(reasoning_output_path, reasoning_filename)
        with open(reasoning_file_path, 'w', encoding='utf-8') as f:
            f.write(reasoning_path)
        #print(f"✓ 已保存推理路径到: {reasoning_file_path}")
    else:
        print(f"✗ 未找到推理路径")
    
    # 将答案保存到单独的文件
    if final_answer:
        answer_file_path = os.path.join(answer_output_path, answer_filename)
        with open(answer_file_path, 'w', encoding='utf-8') as f:
            f.write(final_answer)
        #print(f"✓ 已保存答案到: {answer_file_path}")
    else:
        print(f"✗ 未找到答案")
    
    #print("-" * 50)

def main():
    """主函数，解析命令行参数并开始处理"""
    parser = argparse.ArgumentParser(description='从日志文件中提取推理路径和最终答案')
    parser.add_argument('--input', required=True, help='日志文件或包含日志文件的目录路径')
    parser.add_argument('--reasoning_output', required=True, help='保存推理路径的目录')
    parser.add_argument('--answer_output', required=True, help='保存最终答案的目录')
    parser.add_argument('--dataset_name', default='amc23', help='数据集名称，默认为amc23')
    
    args = parser.parse_args()
    
    process_log_files(args.input, args.reasoning_output, args.answer_output, args.dataset_name)
    
if __name__ == '__main__':
    main()